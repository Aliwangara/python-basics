...
# new_store = {}
# for store in (freelancers, antiques, pet_shop):new_store.update(store)
# print(new_store) YOU CAN USE THIS OR :

new_store = [freelancers, antiques, pet_shop]
...
for store in new_store:
    for key , value in store.items():
       if key != "name":
           department_store[key] = value



print("Inventory Before Shopping")

print(department_store,"\n")

for shop in new_store:
    shop_name = shop["name"]
    ...
    choice = input("What do you want to buy? (type item name, 'exit' to skip) → ").lower().strip()

    if choice == "exit":
        print("You leave the shop.\n")
        break
    if choice not in shop or choice == "name":
        print("Item not found... you leave the shop.\n")
        continue

    price = shop.pop(choice)
    cart[choice] = price
    purse -= price
...
print(f"You purchased: {list(cart.keys())}")
print(f"Total spent: {sum(cart.values())} gold")
print(f"Gold left in purse: {purse}")
...
print("\nInventory AFTER shopping:")
print(department_store)


#loop through stores/dicts
# for key, freelancer in freelancers.items():
#     #inputbox  to show what you can buy...capture textstring of what was bought...make lowercase
#     # buy_item = input(f'Welcome to {freelancers['name']}! what do you want to buy: {list(freelancers.items())[1:]}').lower().strip()
#     print(key, freelancer)
# #     #update the cart
#     cart.update({key:freelancer}) # use pop...
